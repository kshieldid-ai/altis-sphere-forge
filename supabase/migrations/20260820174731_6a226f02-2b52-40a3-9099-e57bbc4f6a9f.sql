...
-- has_role is used only inside RLS policies: revoke direct API execution
REVOKE ALL ON FUNCTION public.has_role(uuid, public.app_role) FROM PUBLIC, anon, authenticated;

-- 2. Politiques devis_requests restreintes aux administrateurs
...
ON public.devis_requests FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can update quote requests"
ON public.devis_requests FOR UPDATE TO authenticated
USING (public.has_role(auth.uid(), 'admin'))
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can delete quote requests"
ON public.devis_requests FOR DELETE TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

-- 3. Suppression de l'ancienne fonction SECURITY DEFINER exposée
